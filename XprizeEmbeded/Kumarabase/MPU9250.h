...
#define INDO 0x01  //  default = INFO 0x01
...
public:
    ...
    void sensitivity(Serial &pc, bool show = false)
    {
        ...
        if (show)
        {
            pc.printf("Accelerometer sensitivity is %f LSB/g \t Gyroscope sensitivity is %f LSB/deg/s \t Magnetometer sensitivity is %f LSB/G \n\r", 1.0f / aRes, 1.0f / gRes, 1.0f / mRes);
            ;
        }
    }
    ...
    void calibrateMag(Serial &pc)
    {
        ...
        for (ii = 0; ii < sample_count; ii++)
        {
            ...
            pc.printf("%d\t%d\t%d\t%d\r\n", mag_temp[0], mag_temp[1], mag_temp[2], ii);
        } // end of outer for
        wait(1);
    ...
    // use the madgwick open source version
    void Madgwickupdate()
    {
        // need to convert the gyro measurements
        // need to flip the mag x/y coordinates and invert the z axis to match the acell and gyro axis
        MadgwickAHRSupdate(gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f, ax, ay, az, my, mx, -1.0f * mz, q[0], q[1], q[2], q[3]);
    }
