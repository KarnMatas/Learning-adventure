...
{ 
  ...
  { 
    ...
    if(base_control_flag){
      ...
      float ans = Theta;
      if(Theta - kumara_base_yaw > 30){
        ans = kumara_base_yaw + 30;
      }
      else if(Theta - kumara_base_yaw < -30){
        ans = kumara_base_yaw - 30;
      }

      ang = pid_controller(0.075,0.0,0.6,ans,kumara_base_yaw,pre_error,i_term,d_term);
      /* move and Rotate */
